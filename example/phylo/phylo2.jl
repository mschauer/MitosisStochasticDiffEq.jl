...
θtrue = ([0.5, 1.2], [0.1, 0.1])
const M = -0.001I + [-1.0 1.0; 1.0 -1.0]
...
tree = Tree(S50)
@test sort(setdiff(1:length(tree.Par), tree.Par)) == sort(tree.lids)
...
"""
function DiffEqNoiseProcess.pCN(source::DiffEqNoiseProcess.AbstractNoiseProcess{T,N,Vector{T2},inplace}, ρ;
  ...
  if typeof(source′.W[1]) <: Number
    Wnew = cumsum([zero(source′.W[1]);[sqrt.(t[i+1]-ti).*DiffEqNoiseProcess.wiener_randn(rng,typeof(source′.W[i]))
              for (i,ti) in enumerate(t[1:end-1])]])
  else
    Wnew = cumsum([[zero(source′.W[1])];[sqrt.(t[i+1]-ti).*DiffEqNoiseProcess.wiener_randn(rng,source′.W[i])
              for (i,ti) in enumerate(t[1:end-1])]])
  end

  source′.W = ρ * source′.W + sqrt(one(ρ)-ρ^2) * Wnew
  source′.u = ρ * source′.u + sqrt(one(ρ)-ρ^2) * Wnew
  #NoiseGrid(t, Vector.(W))
  NoiseWrapper(source′,reset=reset,reverse=reverse,indx=indx)
end

## simulate conditioned process using messages

function fwguidtree!(messages, X, guidedsegs, noisesegs, ll, tree::Tree, f, g, θ, rho)
    T = tree.T
    ll .= -1
    ll[1] = 0.0
    if isnothing(noisesegs)
        noisesegs = guidedsegs
    end
    ...
    #    @show i′, i
        ...
        if rho == 0 || !isassigned(noisesegs, i)
            Z = nothing
        else
            if noisesegs[i].W isa DiffEqNoiseProcess.NoiseProcess || noisesegs[i].W isa NoiseGrid
                Z = pCN(noisesegs[2].W, rho)
            else
                Z = pCN(noisesegs[2].W.source, rho)
            end
        end
...
X, guidedsegs, ll, ll_leaves = fwguidtree!(messages, X, guidedsegs, nothing, zeros(tree.n), tree, f, g, θ, rho)
G = typeof(guidedsegs[2])

# small test if pCN works
ll2 = zeros(tree.n)
X, guidedsegs2, ll2, ll_leaves2 = fwguidtree!(messages, X, copy(guidedsegs), nothing, ll2, tree, f, g, θ, 1.0)

@show ll-ll2 # expected: all zeros
# @show hcat(guidedsegs[3].u- guidedsegs2[3].u) # expected: all zeros

#@test guidedsegs[2].W.W == pCN(guidedsegs[2].W, 1.0).source.W

W = guidedsegs[2].W
u = copy(W.u)
W2 = pCN(W, .0)
#@test_broken W.u != W2.source.u

## plotting
function plot(tree, X, guidedsegs, j=1)
    cols = repeat([:blue, :red, :magenta, :orange],2)
    pl = scatter(tree.T, getindex.(X,j),color=:red)
    for i in 2:tree.n
        gg = guidedsegs[i]
...
function mcmc(θ, prior, iters, Xd, tree; dt0 = 0.001, σθ=0.1, rho=0.8)
    X = zeros(𝕏, tree.n) 
    Xprop = zeros(𝕏, tree.n)  # values at nodes
    for id in tree.lids
        X[id] = Xd[id]
        Xprop[id] = Xd[id]
    end
    ...
    θs = [θ]
    n = tree.n
    Q = [i in tree.lids ? WGaussian{(:μ,:Σ,:c)}(Vector(Xd[i]), 0.002Matrix(I(d)), 0.0) : missing for i in eachindex(Xd)]

    global guidedsegs, X, ll, lls
    guidedsegs = Vector{Any}(undef, n) # save all guided segments
    guidedsegsprop = Vector{Any}(undef, n) # save all guided segments
    lls, ll = zeros(n), 0.0
    llprops, llprob = zeros(n), 0.0

    θlin = (B(θ), zeros(d), Σ(θ))
    Q, messages = bwfiltertree!(Q, tree, θlin, dt0)

    X, guidedsegs, lls, ll = fwguidtree!(messages, X, guidedsegs, nothing, lls, tree, f, g, θ, 0.0)
    Xprop, guidedsegsprop, lls, ll = fwguidtree!(messages, Xprop, guidedsegs, nothing, lls, tree, f, g, θ, 0.0)
    ...
    for iter in 1:iters
       ...
       # llprops .= 0
        θprop = (θ[1] + σθ*randn(size(θ[1])...), θ[2])
        Xprop, guidedsegsprop, llprops, llprop = fwguidtree!(messages, Xprop, guidedsegsprop, guidedsegs, llprops, tree, f, g, θprop, rho)
        if mod(iter, 10) == 0 
            println(iter, " acc ", r.(accepted/iter), " θ ", r.(θ[1]), " ll ", r(ll), " llᵒ ", r(llprop))
        end
        if  rand() <= exp(llprop - ll)
            ll = llprop
            θ = θprop
            guidedsegs, guidedsegsprop = guidedsegsprop, guidedsegs
            X, Xprop = Xprop, X
            accepted += 1
        end
        push!(θs, θ)
...
iters = 5000
θinit = (0.9*θtrue[1], θtrue[2])
θs, acc = mcmc(θinit, prior, iters, Xd, tree; σθ=0.01, rho=0.995, dt0=0.001)
